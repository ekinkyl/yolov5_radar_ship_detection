...
for ip, img_path in enumerate(pngs, 1):
    ...
    # --- UPDATED: read unchanged, collapse temporal [prev,cur,next] to grayscale via middle channel
    raw = cv2.imread(str(img_path), cv2.IMREAD_UNCHANGED)
    if raw is None:
        continue
    if raw.ndim == 3 and raw.shape[2] == 3:
        gray = raw[:, :, 1]  # middle frame 'cur'
    else:
        gray = raw
    img = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)  # for colored drawings
    # ---
