...
IMG_DIR = ROOT / "newDataset/images/test"
GT_DIR = ROOT / "newDataset/labels/test"
PRED_DIR = ROOT / "runs/val/yosmr_newDataset_test/labels"
OUT_DIR = ROOT / "runs/val/yosmr_newDataset_test/vis"
...
def load_pred_boxes(txt_path, W, H):
    """YOLO preds (from val.py --save-txt): class xc yc w h [conf] Be tolerant to presence/absence/order of conf."""
    boxes = []
    if not txt_path.exists():
        return boxes
    with open(txt_path) as f:
